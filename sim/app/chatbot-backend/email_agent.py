...
def send_email_tool(to: str, subject: str, body: str) -> str:
    ...
    try:
        response = requests.post("http://localhost:8000/mcp/gmail/send", json=payload)
        ...
        if should_send_email:
            try:
                ...
                if extracted and all(k in extracted for k in ("to", "subject", "body")):
                    print(f"📧 Extracted email details: {extracted}")
            except Exception as e:
...
def should_send_email(state: AgentState) -> str:
    if all(k in state for k in ("to", "subject", "body")):
        print(f"🤖 should_send_email check — state keys: {list(state.keys())}")
        return "send_email"
    return END
...
# Wrapper to ensure correct input to the tool
